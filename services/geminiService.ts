...
export const generatePhotorealisticScene = async (sceneDescription: string): Promise<string> => {
  ...
  try {
    ...
    console.log('[generatePhotorealisticScene] Generating with Imagen 3.0 via REST API');

    // Use Imagen 3.0 REST API directly
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: response.statusText }));
      throw new Error(`Imagen API error: ${error.error || response.statusText}`);
    }

    const result = await response.json();

    if (!result.predictions || result.predictions.length === 0) {
      throw new Error('No images generated by Imagen 3.0');
    }

    const imageData = result.predictions[0].bytesBase64Encoded;
    return `data:image/png;base64,${imageData}`;
  } catch (error: any) {
    console.error('[generatePhotorealisticScene] Error:', error);
